...
- **Requirements and scope questions** — "what does X mean?", "which 特性 are in scope?"
...
**Other environments:** The server must keep running in the background across conversation turns. If your environment reaps detached processes, use `--foreground` and launch the command with your 平台's background execution mechanism.

If the URL is unreachable from your browser (常见 in remote/containerized setups), bind a non-loopback host:
...
3. **On your next turn** — after the user responds in the terminal:
   - Read `$STATE_DIR/events` if it exists — this contains the user's browser interactions (点击, selections) as JSON lines
...
When the user 点击 options in the browser, their interactions are recorded to `$STATE_DIR/events` (one JSON object per line). The file is cleared automatically when you push a new screen.
...
The full 事件 stream shows the user's exploration path — they may click multiple options before settling. The last `choice` 事件 is typically the final selection, but the pattern of 点击 can reveal hesitation or preferences worth asking about.
...
If the session used `--project-dir`, mockup files persist in `.superpowers/brainstorm/` for later 参考. Only `/tmp` sessions get deleted on stop.
